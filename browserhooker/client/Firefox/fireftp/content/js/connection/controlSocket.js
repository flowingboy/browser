...
ftpMozilla.prototype = {
  ...
  version              : "1.0.4",  // version of this class - used to avoid collisions in cache
  ...
  sftpKill : function() {
    ...
    try {
      if (this.getPlatform() == "windows") {
        ...
        var killPath = "/usr/bin/killall psftp";
        this.ipcService.exec(killPath);
      }
    } catch (ex) { }
  ...
  cacheHit : function(path, callback) {
    try {                                                                      // check the cache first
      ...
      if (cacheDesc.dataSize) {
        ...
        if (this.observer) {
          this.observer.onDebug(this.listData.toSource().replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/, {/g, ',<br/>{')
