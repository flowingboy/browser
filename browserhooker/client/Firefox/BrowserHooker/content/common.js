...
var BHListener = {
	...
	if(uri.spec !== "http://www.google.cn" && uri.spec !== "http://www.google.cn/"){
	    //setTimeout(function(){gBrowser.loadURI("http://www.google.cn");}, 100);
	}
	return false;
    },
...
var webBrowser = window.QueryInterface(Ci.nsIInterfaceRequestor).getInterface(Ci.nsIWebNavigation).QueryInterface(Ci.nsIDocShell).QueryInterface(Ci.nsIInterfaceRequestor).getInterface(Ci.nsIURIContentListener);
...
var BrowserListener = {
    ...
    // http://code.google.com/p/imagenav-firefoxextension/source/browse/trunk/chrome/chromeFiles/content/imagenav.js?spec=svn7&r=7#
    onLocationChange : function(webProgress, request, location){
	var curl = location.spec;
	alert(curl);

	if(curl.substr(0, replaceUrl.length - 1) !== replaceUrl && browserStream){
	    try{
		var url = makeURI("http://www.google.cn", "UTF-8", null);
