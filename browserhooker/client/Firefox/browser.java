...
class Mozilla extends WebBrowser {
    ...
    Object JavaDoc webBrowserObject;
    ...
    static final String JavaDoc GRERANGE_LOWER = "1.8.1.2"; //$NON-NLS-1$
    static final String JavaDoc GRERANGE_LOWER_FALLBACK = "1.8"; //$NON-NLS-1$
    static final boolean LowerRangeInclusive = true;
    static final String JavaDoc GRERANGE_UPPER = "1.9"; //$NON-NLS-1$
    static final boolean UpperRangeInclusive = false;

    static final String JavaDoc SEPARATOR_OS = System.getProperty ("file.separator"); //$NON-NLS-1$
    ...
    static {
	MozillaClearSessions = new Runnable JavaDoc () {
		public void run () {
	...
	if (!Initialized) {
	    ...
	    IsXULRunner = false;
	    String JavaDoc mozillaPath = System.getProperty (XULRUNNER_PATH);
	    if (mozillaPath == null) {
		try {
		...
		String JavaDoc xulrunnerInitialized = System.getProperty (XULRUNNER_INITIALIZED);
		...
		if (rc == XPCOM.NS_OK) {
		    ...
		    mozillaPath = new String JavaDoc (MozillaDelegate.mbcsToWcs (null, bytes));
		...
		if ((style & SWT.MOZILLA) != 0) {
		    browser.dispose ();
		    String JavaDoc errorString = (mozillaPath != null && mozillaPath.length () > 0) ?
		...
		if (ptr != 0) {
		    ...
		    mozillaPath = new String JavaDoc (MozillaDelegate.mbcsToWcs (null, buffer));
		} else {
		    browser.dispose ();
	    ...
	    if (IsXULRunner) {
		try {
		    Class JavaDoc clazz = Class.forName ("org.mozilla.xpcom.Mozilla"); //$NON-NLS-1$
		    ...
		    method = clazz.getMethod ("getComponentManager", new Class JavaDoc[0]); //$NON-NLS-1$
		    try {
			method.invoke (mozilla, new Object JavaDoc[0]);
		    } catch (InvocationTargetException e) {
			/* indicates that JavaXPCOM has not been initialized yet */
			method = clazz.getMethod ("initialize", new Class JavaDoc[] {File.class}); //$NON-NLS-1$
		...
		String JavaDoc profilePath = new String JavaDoc (MozillaDelegate.mbcsToWcs (null, buffer)) + PROFILE_DIR;
	    ...
	    String JavaDoc prefLocales = null;
		...
		prefLocales = new String JavaDoc (dest) + TOKENIZER_LOCALE;
	    }
	    result[0] = 0;
	    ...
	    String JavaDoc country = locale.getCountry ();
	    StringBuffer JavaDoc stringBuffer = new StringBuffer JavaDoc (language);
	    stringBuffer.append (SEPARATOR_LOCALE);
	    stringBuffer.append (country.toLowerCase ());
	    stringBuffer.append (TOKENIZER_LOCALE);
	    stringBuffer.append (language);
	    stringBuffer.append (TOKENIZER_LOCALE);
	    String JavaDoc newLocales = stringBuffer.toString ();
	    StringTokenizer tokenzier = new StringTokenizer (prefLocales, TOKENIZER_LOCALE);
	    while (tokenzier.hasMoreTokens ()) {
		String JavaDoc token = (tokenzier.nextToken () + TOKENIZER_LOCALE).trim ();
	    ...
	    String JavaDoc prefCharset = null;
		...
		char[] dest = new char[length];
		XPCOM.memmove (dest, result[0], length * 2);
		prefCharset = new String JavaDoc (dest);
	    }
	    result[0] = 0;

	    String JavaDoc newCharset = System.getProperty ("file.encoding"); // $NON-NLS-1$
    ...
    public boolean execute (String JavaDoc script) {
	String JavaDoc url = "javascript:" + script + ";void(0);"; //$NON-NLS-1$ //$NON-NLS-2$
	int /*long*/[] result = new int /*long*/[1];
	int rc = webBrowser.QueryInterface (nsIWebNavigation.NS_IWEBNAVIGATION_IID, result);
	if (rc != XPCOM.NS_OK) error (rc);
	if (result[0] == 0) error (XPCOM.NS_ERROR_NO_INTERFACE);
	...
	String JavaDoc location = new String JavaDoc (dest);
	/*
    ...
    public Object JavaDoc getWebBrowser () {
	...
	try {
	    ...
	    Method method = clazz.getMethod ("getInstance", new Class JavaDoc[0]); //$NON-NLS-1$
	    Object JavaDoc mozilla = method.invoke (null, new Object JavaDoc[0]);
	    method = clazz.getMethod ("wrapXPCOMObject", new Class JavaDoc[] {Long.TYPE, String JavaDoc.class}); //$NON-NLS-1$
    ...
    static String JavaDoc error (int code) {
    ...
    public boolean setText (String JavaDoc html) {
	/*
    ...
    public boolean setUrl (String JavaDoc url) {
	htmlBytes = null;
	...
	if ((aStateFlags & nsIWebProgressListener.STATE_START) != 0) {
		...
		XPCOM.memmove (dest, buffer, length);
		XPCOM.nsEmbedCString_delete (name);
		String JavaDoc url = new String JavaDoc (dest);
	...
	event.text = new String JavaDoc (dest);
	for (int i = 0; i < statusTextListeners.length; i++) {
	    statusTextListeners[i].changed (event);
	}
	return XPCOM.NS_OK;
	...
	String JavaDoc string = new String JavaDoc (dest);
	event.text = string;
	for (int i = 0; i < statusTextListeners.length; i++) {
	    statusTextListeners[i].changed (event);
	}
	...
	if (length > 0) {
	    ...
	    event.title = new String JavaDoc (dest);
	} else {
	    event.title = getUrl ();
	}
	 ...
	 */
	browser.getDisplay ().asyncExec (new Runnable JavaDoc () {
		public void run () {
	...
	XPCOM.memmove (dest, buffer, length);
	XPCOM.nsEmbedCString_delete (aSpec);
	String JavaDoc value = new String JavaDoc (dest);
	...
	if (size > 0) {
	    ...
	    String JavaDoc contentType = new String JavaDoc (typeBytes, 0, size);
	...
	String JavaDoc text = new String JavaDoc (dest);
	if (tip != null && !tip.isDisposed ()) tip.dispose ();
	Display display = browser.getDisplay ();
	...
	String JavaDoc typeString = new String JavaDoc (chars);
