...
BOOL IOCPS::Startup()
{
	...
	if ( !m_bServerStarted )
	{
		...
		*/
		...
		// we have some sort of bugg somewhere.. 
		// @Modified by flowingboy 2009-2-16
		// @Add " || m_iMaxIOWorkers < 1"
		if ( m_iMaxIOWorkers > 1 || m_iMaxIOWorkers < 1) 
		...
		while ( !bError )
		{
			...
			if (!bIORet)  
			{
				...
				{	
					...
					bError = true;
					pThis->AppendLog(pThis->ErrorCode2Text(dwIOError)); 
					pThis->AppendLog("IoWorker is killed because of error in GetQueuedCompletionStatus");
			...
			if(bIORet && lpOverlapped && lpClientContext) 
			{
				pOverlapBuff=CONTAINING_RECORD(lpOverlapped, CIOCPBuffer, m_ol);
				CString msg;
				msg.Format("The IoSize is %d", dwIoSize);
				if(pOverlapBuff!=NULL)
	...
	if(pContext!=NULL)
	{
		...
		*/
		...
		{	
			...
			if(pOverlapBuff!=NULL)
			{
				// PostQueuedCompletionStatus is part of IOCP - flowingboy
...
// ½«ÊÂÏÈ´´½¨ºÃµÄsocketÓëÍê³É¶Ë¿Ú°ó¶¨ÆðÀ´
...
void IOCPS::OnPostedPackage(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	...
	if ( pOverlapBuff)
	{
		...
		nSize = strlen((const char*)pOverlapBuff->GetBuffer());
		pContext->m_ContextLock.Lock();
...
// in this function trigger the OnSend function
void IOCPS::OnReadCompleted(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	msg.Format("Enter OnReadCompleted! and the data received is: %s\n", pOverlapBuff->GetBuffer());
	AppendLog(msg);
...
void IOCPS::ProcessPackage(ClientContext *pContext, DWORD dwIoSize, CIOCPBuffer *pOverlapBuff)
{
	...
	if ( pBuffPartialMessage!=NULL )
	{
		...
		if ( nUsedBuffer >= MINIMUMPACKAGESIZE )
		{
			...
			nSize = strlen((const char*)pBuffPartialMessage->GetBuffer());
	...
	do 
	{
		...
		{
			UINT nSize = 0;
			// nSizeµÄÖµ²»¶Ô£¬ËÆºõÊÇ¸ºÊý
			//memmove(&nSize,pOverlapBuff->GetBuffer(),MINIMUMPACKAGESIZE);
			nSize = strlen((const char*)pOverlapBuff->GetBuffer());
			CString msg;
			msg.Format("%sThe nSize value is: %d", (LPCTSTR)pOverlapBuff->GetBuffer(), nUsedBuffer);
			AppendLog(msg);
...
// flowingboy : this function seems to be buggable.
