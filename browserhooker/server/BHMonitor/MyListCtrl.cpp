...
// changed by zyudong 2009-04-23
BOOL MyListCtrl::init(LPCTSTR sHeaderStrArr[], int iHeaderStrArrLen, int iColWidthArr[], int iColWidthArrLen)
{
	if(iHeaderStrArrLen < 1
	|| sHeaderStrArr == NULL
	|| iColWidthArrLen < 1
	|| iColWidthArr == NULL
	|| iHeaderStrArrLen != iColWidthArrLen
	)
		return FALSE;
	...
	m_iNumberOfColumns = iColWidthArrLen;

	m_iColumnWidthArray[0]=20;
	m_iColumnWidthArray[1]=20;
	m_iColumnWidthArray[2]=30;

	for(int i = 0; i < iHeaderStrArrLen; i++){
		InsertColumn(i, sHeaderStrArr[i], LVCFMT_LEFT, iColWidthArr[i]);
	}

	// We whant a nice flat list-  	
	SetExtendedStyle(LVS_EX_GRIDLINES | LVS_EX_FLATSB | LVS_EX_FULLROWSELECT );
